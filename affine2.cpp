...
void backtracking (vector< vector<int> > &V, vector< vector<int> > &E, vector< vector<int> > &F, string &seqA, string &seqB) {
 ...
 void  NWalignSeqPairs(string filename, vector< vector<string > > &align, vector< vector<string > > &Mnames) {
...
vector< vector<string > > ChosenSeqPairs(string filename1, vector< vector<string > > &align, vector< vector<string > > &Mnames, string Name)
{
...
for( int j = 0; j < Mnames.size(); j+=1) {	
	for( int i = 0; i < Mnames[0].size(); i+=1) {
	   if (Mnames[i][j]==Name) {
	     auxChalign.push_back(align[i][0]);	auxChalign.push_back(align[i][1]);	 
		 fout<<"Sequences: "<<Mnames[i][0]<<", "<<Mnames[i][1]<<endl;
		 fout<<align[i][0]<<endl<<align[i][0]<<endl;   
	     }     
...
int main () {
	...
	vector< vector<string > > Mnames;
   	NWalignSeqPairs(filename, align, Mnames);
   	Chalign=ChosenSeqPairs(filename1, align, Mnames, "S2");   
